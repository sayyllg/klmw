 ...
 */
namespace Ehr\Model;
use Ehr\Model\EhrModel;
use Ehr\Db\LgDb;

class LgModel extends EhrModel
{
	private $_dblg	= NULL;
	...
	public function save( $data ) 
	{
		...
		foreach ($data['body_params'] as $key => $val) {
			...
			$body['static_times'] = 1;
			if ($val['static_times'] > 1) {
				for ($i=0; $i < $val['static_times'] ; $i++) { 
					$body['upload_at'] = $val['upload_at']+(($i+1)*60);
					$body_params[] = $body;
				}
			}else{
