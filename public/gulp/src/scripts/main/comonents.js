Ehr.module('Components', function(Components, Ehr, Backbone, Marionette, $, _){
	...
	*/
	...
	initialize: function(options) {
	  ...
	  Ehr.Components.Form.validators.errMessages = {
	    required: '不能为空',
	    regexp: '无效的格式',
	    number: '必须为数字',
	    email: '错误的邮箱格式',
	    url: '无效的URL地址',
	    match: _.template('Must match field "<%= field %>"', null, Ehr.Components.Form.templateSettings)
	  };
	...
	setButtonState: function() {
	  ...
	  ? this.submitButton.removeClass('disabled').removeAttr('disabled').html( this.options.submitButton )
	  : this.submitButton.addClass('disabled').attr('disabled', 'disabled')
	    && this.options.processing && this.submitButton.html('<i class="fa fa-refresh fa-spin"></i> '+this.options.processing)
	},
	render: function() {
	...
	renderOptions: function(options) {
	  ...
	  if(options instanceof Backbone.Collection){
	    ...
	    if(!!options[0]){
	      ...
	      options = _.map(options, function(val){ return {val:val[ll.val],label:val[ll.label]} });
	    }else{
	...
	*/
	...
	_initAtWho: function(){
	  var emojis = $.map(Components.Dicts.emojis, function(value, i) {return {key: value, name:value}});
	  var self = this;
	  var at_config = {
	    at: "@",
	    callbacks: {
	      remote_filter: function(query, callback) {
	        // if(query.length !=0){
	          self.collection.fetch({data:{s:query},reset:true,success:function(collection,resp){
	...
	*/
	...
	// 	});
		...
		if( this.uploader && _.filter(this.files.models, function(item){ return item.get('uploading')}).length!=0){
